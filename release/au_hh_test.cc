...
TEST(AuHh, PrintsValueAndUnitLabel) {
    EXPECT_THAT(stream_to_string(meters(3)), StrEq("3 m"));
    EXPECT_THAT(stream_to_string((meters / milli(second))(1.25)), StrEq("1.25 m / ms"));
}
...
TEST(AuHh, PrintsZero) { EXPECT_THAT(stream_to_string(ZERO), StrEq("0")); }
