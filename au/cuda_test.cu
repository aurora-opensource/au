...
//
// Compile with: nvcc -std=c++14 -I. --compile au/cuda_test.cu
//
...
__device__ double test_quantity_coerce_as() {
    auto q = meters(5.0);
    return q.coerce_as(centi(meters)).in(centi(meters));
}

__device__ double test_quantity_coerce_as_rep() {
    auto q = meters(5.0);
    return q.coerce_as<int>(centi(meters)).in(centi(meters));
}

__device__ double test_quantity_coerce_in() {
    auto q = meters(5.0);
    return q.coerce_in(centi(meters));
}

__device__ double test_quantity_coerce_in_rep() {
    auto q = meters(5.0);
    return static_cast<double>(q.coerce_in<int>(centi(meters)));
}
...
__device__ double test_quantity_point_coerce_as() {
    auto p = meters_pt(5.0);
    return p.coerce_as(centi(meters_pt)).in(centi(meters_pt));
}

__device__ double test_quantity_point_coerce_in() {
    auto p = meters_pt(5.0);
    return p.coerce_in(centi(meters_pt));
}
...
__device__ double test_constant_coerce_as() {
    return SPEED_OF_LIGHT.coerce_as<double>(meters / second).in(meters / second);
}

__device__ double test_constant_coerce_in() {
    return SPEED_OF_LIGHT.coerce_in<double>(meters / second);
}
